...
public class ProjectSkillMappingController {
    ...
    private final ProjectSkillMappingService projectSkillMappingService;

    @Autowired
    public ProjectSkillMappingController(ProjectSkillMappingService projectSkillMappingService) {
        this.projectSkillMappingService = projectSkillMappingService;
    }
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<ApiResponse<List<UUID>>> getRecommendedStudentsForProject(@PathVariable UUID projectId) {
